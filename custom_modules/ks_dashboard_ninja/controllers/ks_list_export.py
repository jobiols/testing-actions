...
class KsListExport(ExportFormat, http.Controller):
    ...
    def base(self, data, token):
        ...
        header, list_data, item_id, ks_export_boolean, context, params = operator.itemgetter(
            'header', 'chart_data', 'ks_item_id', 'ks_export_boolean',
        ...
        if ks_export_boolean:
            ...
            if item._context.get('ksDateFilterSelection', False):
                ks_date_filter_selection = item._context[
                ...
                item = item.with_context(
                    ksDateFilterStartDate=item.ks_dashboard_ninja_board_id.
                    ks_dashboard_start_date)
                item = item.with_context(
                    ksDateFilterEndDate=item.ks_dashboard_ninja_board_id.
            ...
            if list_data['type'] == 'ungrouped':
                list_data = item.ks_fetch_list_view_data(item,
                                                         ks_chart_domain,
                                                         ks_export_all=True)
            elif list_data['type'] == 'grouped':
                list_data = item.get_list_view_record(orderby,
                                                      sort_order,
                                                      ks_chart_domain,
                                                      ks_export_all=True)
...
class KsListExcelExport(KsListExport, http.Controller):
    ...
    @http.route('/ks_dashboard_ninja/export/list_xls',
                type='http',
                auth="user")
    @serialize_exception
...
class KsListCsvExport(KsListExport, http.Controller):
    ...
    @http.route('/ks_dashboard_ninja/export/list_csv',
                type='http',
                auth="user")
    @serialize_exception
    def index(self, data, token):
