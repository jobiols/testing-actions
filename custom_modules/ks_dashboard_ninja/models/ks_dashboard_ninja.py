        ...
        if ks_dashboard_rec.ks_child_dashboard_ids:
            ...
            if selecred_rec:
                ...
                dashboard_data["ks_gridstack_config"] = (
                    ks_dashboard_rec.ks_child_dashboard_ids.search(
                        [["ks_active", "=", True]], limit=1
                    ).ks_gridstack_config,
                )
            else:
    ...
    def ks_import_dashboard(self, file):
        ...
        for data in ks_dashboard_data:
            ...
            vals = {
                ...
                "ks_dashboard_top_menu_id": ks_dashboard_top_menu_id.id
                if ks_dashboard_top_menu_id
    ...
    def ks_prepare_item(self, item):
        ...
        item["ks_item_start_date"] = (
            datetime.datetime.strptime(
                item["ks_item_start_date"].split(" ")[0], "%Y-%m-%d"
            )
            if item["ks_item_start_date"]
            else False
        )
        item["ks_item_end_date"] = (
            datetime.datetime.strptime(
                item["ks_item_end_date"].split(" ")[0], "%Y-%m-%d"
            )
            if item["ks_item_end_date"]
            else False
        )
        item["ks_item_start_date_2"] = (
            datetime.datetime.strptime(
                item["ks_item_start_date_2"].split(" ")[0], "%Y-%m-%d"
            )
            if item["ks_item_start_date_2"]
            else False
        )
        item["ks_item_end_date_2"] = (
            datetime.datetime.strptime(
                item["ks_item_end_date_2"].split(" ")[0], "%Y-%m-%d"
            )
            if item["ks_item_end_date_2"]
            else False
        )
