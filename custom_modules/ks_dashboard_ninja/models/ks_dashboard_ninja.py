...
class KsDashboardNinjaBoard(models.Model):
    ...
    ks_dashboard_items_ids = fields.One2many('ks_dashboard_ninja.item',
    ...
    ks_dashboard_group_access = fields.Many2many('res.groups',
                                                 string="Group Access")
    ...
    ks_data_formatting = fields.Selection([('global', 'Global'),
                                           ('indian', 'Indian'),
                                           ('exact', 'Exact')],
                                          string='Format')
    ...
    ks_dashboard_menu_sequence = fields.Integer(
        ...
        "low priority")
    ks_child_dashboard_ids = fields.One2many('ks_dashboard_ninja.child_board',
                                             'ks_dashboard_ninja_id')
    ...
    def create(self, vals):
        ...
        if 'ks_dashboard_top_menu_id' in vals and 'ks_dashboard_menu_name' in vals:
            ...
            record.ks_dashboard_client_action_id = self.env[
                'ir.actions.client'].sudo().create(action_id)

            record.ks_dashboard_menu_id = self.env['ir.ui.menu'].sudo().create(
                {
                    'name':
                    vals['ks_dashboard_menu_name'],
                    'active':
                    vals.get('ks_dashboard_active', True),
                    'parent_id':
                    vals['ks_dashboard_top_menu_id'],
                    'action':
                    "ir.actions.client," +
                    str(record.ks_dashboard_client_action_id.id),
                    'groups_id':
                    vals.get('ks_dashboard_group_access', False),
                    'sequence':
                    vals.get('ks_dashboard_menu_sequence', 10)
                })
        ...
        if record.ks_dashboard_default_template and record.ks_dashboard_default_template.ks_item_count:
            ...
            for item_data in template_data:
                if record.ks_dashboard_default_template.ks_template_type == 'ks_custom':
                    dashboard_item = self.env[
                        'ks_dashboard_ninja.item'].browse(int(item_data)).copy(
                            {'ks_dashboard_ninja_board_id': record.id})
                    ks_gridstack_config[
                        dashboard_item.id] = template_data[item_data]
                else:
    ...
    def write(self, vals):
        if vals.get(
                ...
                False) and vals.get('ks_date_filter_selection') != 'l_custom':
            vals.update({
        ...
        for rec in self:
            if 'ks_dashboard_menu_name' in vals:
                if self.env.ref(
                        'ks_dashboard_ninja.ks_my_default_dashboard_board'
                ) and self.env.ref(
                        'ks_dashboard_ninja.ks_my_default_dashboard_board'
                ).sudo().id == rec.id:
                    if self.env.ref('ks_dashboard_ninja.board_menu_root',
                                    False):
                        self.env.ref(
                            'ks_dashboard_ninja.board_menu_root').sudo(
                            ).name = vals['ks_dashboard_menu_name']
                else:
                    rec.ks_dashboard_menu_id.sudo(
                    ).name = vals['ks_dashboard_menu_name']
            ...
            if 'ks_dashboard_active' in vals and rec.ks_dashboard_menu_id:
                rec.ks_dashboard_menu_id.sudo(
                ).active = vals['ks_dashboard_active']
            ...
            if 'ks_dashboard_menu_sequence' in vals:
                rec.ks_dashboard_menu_id.sudo(
                ).sequence = vals['ks_dashboard_menu_sequence']

        return record

    def unlink(self):
        if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board'
                        ).id in self.ids:
    ...
    def ks_fetch_dashboard_data(self, ks_dashboard_id, ks_item_domain=False):
        ...
        self = self.with_context(ks_dn_active_ids=ks_dn_active_ids, )
        ...
        dashboard_data = {
            ...
            'ks_date_filter_selection':
            self._context.get('ksDateFilterSelection', False)
            or self.browse(ks_dashboard_id).ks_date_filter_selection,
            'ks_gridstack_config':
            ks_dashboard_rec.ks_gridstack_config,
            'ks_set_interval':
            ks_dashboard_rec.ks_set_interval,
            'ks_data_formatting':
            ks_dashboard_rec.ks_data_formatting,
            'ks_dashboard_items_ids':
            ks_dashboard_rec.ks_dashboard_items_ids.ids,
            'ks_item_data': {},
            'ks_child_boards':
            False,
            'ks_selected_board_id':
            False,
            'ks_dashboard_domain_data':
            ks_dashboard_rec.ks_prepare_dashboard_domain(),
            'ks_dashboard_pre_domain_filter':
            ks_dashboard_rec.ks_prepare_dashboard_pre_domain(),
            'ks_dashboard_custom_domain_filter':
            ks_dashboard_rec.ks_prepare_dashboard_custom_domain(),
            'ks_item_model_relation':
            dict([(x['id'], [x['ks_model_name'], x['ks_model_name_2']])
        ...
        if ks_dashboard_rec.ks_child_dashboard_ids:
            dashboard_data['ks_child_boards'] = {
                'ks_default':
                [ks_dashboard_rec.name, ks_dashboard_rec.ks_gridstack_config]
            }
            ...
            if selecred_rec:
                ...
                dashboard_data[
                    'ks_gridstack_config'] = ks_dashboard_rec.ks_child_dashboard_ids.search(
                        [['ks_active', '=', True]],
                        limit=1).ks_gridstack_config,
            else:
            ...
            for rec in self.env['ks_dashboard_ninja.child_board'].search_read(
                ...
                dashboard_data['ks_child_boards'][str(
                    rec['id'])] = [rec['name'], rec['ks_gridstack_config']]
        ks_item_domain = ks_item_domain or []
        try:
            items = self.ks_dashboard_items_ids.search(
                [['ks_dashboard_ninja_board_id', '=', ks_dashboard_id]] +
                ks_item_domain).ids
    ...
    def ks_fetch_item_data(self, rec, params={}):
        ...
        item = {
            ...
            rec.ks_model_id.name if rec.ks_model_id else "Name",
            'ks_background_color':
            rec.ks_background_color,
            'ks_font_color':
            rec.ks_font_color,
            'ks_header_bg_color':
            ...
            'ks_domain':
            rec.ks_convert_into_proper_domain(rec.ks_domain, rec,
                                              item_domain1),
            'ks_dashboard_id':
            rec.ks_dashboard_ninja_board_id.id,
            'ks_icon':
            rec.ks_icon,
            'ks_model_id':
            rec.ks_model_id.id,
            'ks_model_name':
            rec.ks_model_name,
            'ks_model_display_name':
            rec.ks_model_id.name,
            'ks_record_count_type':
            rec.ks_record_count_type,
            'ks_record_count':
            rec._ksGetRecordCount(item_domain1),
            'id':
            rec.id,
            'ks_layout':
            rec.ks_layout,
            ...
            rec.ks_default_icon_color,
            # Pro Fields
            'ks_dashboard_item_type':
            rec.ks_dashboard_item_type,
            'ks_chart_item_color':
            rec.ks_chart_item_color,
            ...
            'ks_record_field':
            rec.ks_record_field.id if rec.ks_record_field else False,
            'ks_chart_data':
            rec._ks_get_chart_data(item_domain1),
            'ks_list_view_data':
            rec._ksGetListViewData(item_domain1),
            'ks_chart_data_count_type':
            rec.ks_chart_data_count_type,
            'ks_bar_chart_stacked':
            rec.ks_bar_chart_stacked,
            'ks_semi_circle_chart':
            rec.ks_semi_circle_chart,
            'ks_list_view_type':
            rec.ks_list_view_type,
            'ks_list_view_group_fields':
            rec.ks_list_view_group_fields.ids
            if rec.ks_list_view_group_fields else False,
            'ks_previous_period':
            rec.ks_previous_period,
            'ks_kpi_data':
            rec._ksGetKpiData(item_domain1, item_domain2),
            'ks_goal_enable':
            rec.ks_goal_enable,
            'ks_model_id_2':
            rec.ks_model_id_2.id,
            'ks_record_field_2':
            rec.ks_record_field_2.id,
            'ks_data_comparison':
            rec.ks_data_comparison,
            'ks_target_view':
            rec.ks_target_view,
            'ks_date_filter_selection':
            rec.ks_date_filter_selection,
            'ks_show_data_value':
            rec.ks_show_data_value,
            'ks_update_items_data':
            rec.ks_update_items_data,
            'ks_show_records':
            rec.ks_show_records,
            ...
            0,
            'max_sequnce':
            len(rec.ks_action_lines) if rec.ks_action_lines else False,
            'action':
            action,
            'ks_hide_legend':
            rec.ks_hide_legend,
            'ks_data_calculation_type':
            rec.ks_data_calculation_type,
            'ks_export_all_records':
            rec.ks_export_all_records,
            'ks_data_formatting':
            rec.ks_data_format,
            'ks_auto_update_type':
            rec.ks_auto_update_type,
            'ks_show_live_pop_up':
            rec.ks_show_live_pop_up,
            'ks_is_client_action':
            rec.ks_is_client_action,
            'ks_pagination_limit':
            rec.ks_pagination_limit,
            'ks_record_data_limit':
            rec.ks_record_data_limit,
            'ks_chart_cumulative_field':
            rec.ks_chart_cumulative_field.id,
            'ks_chart_cumulative':
            rec.ks_chart_cumulative,
            'ks_button_color':
            rec.ks_button_color,
            'ks_to_do_data':
            rec._ksGetToDOData(),
            'ks_multiplier_active':
            rec.ks_multiplier_active,
            'ks_multiplier':
            rec.ks_multiplier,
            'ks_goal_liness':
            True if rec.ks_goal_lines else False,
            'ks_currency_symbol':
            ks_currency_symbol,
            'ks_currency_position':
            ks_currency_position
        }
        return item
    ...
    @api.model
    def ks_get_list_view_data_offset(self,
    ...
    def ks_view_items_view(self):
        ...
        return {
            ...
            'search_view_id':
            self.env.ref('ks_dashboard_ninja.ks_item_search_view').id,
            'context': {
    ...
    def ks_export_item_data(self, rec):
        ...
        for res in rec.ks_goal_lines:
            goal_line = {
                'ks_goal_date':
                datetime.datetime.strftime(res.ks_goal_date, "%Y-%m-%d"),
                'ks_goal_value':
                res.ks_goal_value,
            }
        ...
        for res in rec.ks_dn_header_lines:
            ...
            if res.ks_to_do_description_lines:
                ...
                ks_dn_header_line[
                    res.ks_to_do_header] = ks_to_do_description_lines
        ...
        item = {
            ...
            rec.ks_domain,
            'ks_icon':
            str(rec.ks_icon) if rec.ks_icon else False,
            'ks_id':
            rec.id,
            'ks_model_id':
            rec.ks_model_name,
            ...
            rec.ks_date_filter_selection,
            'ks_item_start_date':
            rec.ks_item_start_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_start_date else False,
            'ks_item_end_date':
            rec.ks_item_end_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_end_date else False,
            'ks_date_filter_selection_2':
            rec.ks_date_filter_selection_2,
            'ks_item_start_date_2':
            rec.ks_item_start_date_2.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_start_date_2 else False,
            'ks_item_end_date_2':
            rec.ks_item_end_date_2.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            if rec.ks_item_end_date_2 else False,
            'ks_previous_period':
            rec.ks_previous_period,
            'ks_target_view':
            rec.ks_target_view,
            ...
            rec.ks_record_count_type_2,
            'ks_record_field_2':
            rec.ks_record_field_2.name,
            'ks_model_id_2':
            rec.ks_model_id_2.model,
            'ks_date_filter_field_2':
            rec.ks_date_filter_field_2.name,
            'ks_action_liness':
            ks_action_lines,
            'ks_compare_period':
            rec.ks_compare_period,
            'ks_year_period':
            rec.ks_year_period,
            'ks_compare_period_2':
            rec.ks_compare_period_2,
            'ks_year_period_2':
            rec.ks_year_period_2,
            'ks_domain_2':
            rec.ks_domain_2,
            'ks_show_data_value':
            rec.ks_show_data_value,
            ...
            rec.ks_list_target_deviation_field.name,
            'ks_unit':
            rec.ks_unit,
            'ks_show_records':
            rec.ks_show_records,
            ...
            rec.ks_hide_legend,
            'ks_fill_temporal':
            rec.ks_fill_temporal,
            'ks_domain_extension':
            rec.ks_domain_extension,
            'ks_unit_selection':
            rec.ks_unit_selection,
            'ks_chart_unit':
            rec.ks_chart_unit,
            'ks_bar_chart_stacked':
            rec.ks_bar_chart_stacked,
            ...
            'ks_actions':
            rec.ks_actions.xml_id if rec.ks_actions else False,
            'ks_client_action':
            rec.ks_client_action.xml_id if rec.ks_client_action else False,
            'ks_is_client_action':
            rec.ks_is_client_action,
            'ks_export_all_records':
            rec.ks_export_all_records,
            ...
            'ks_multiplier':
            rec.ks_multiplier,
            'ks_record_data_limit_visibility':
    ...
    def ks_dashboard_export(self, ks_dashboard_ids):
        ...
        for ks_dashboard_id in ks_dashboard_ids:
            ...
            dashboard_data = {
                ...
                ks_dashboard_rec.ks_dashboard_menu_name,
                'ks_gridstack_config':
                ks_dashboard_rec.ks_gridstack_config,
                'ks_set_interval':
                ks_dashboard_rec.ks_set_interval,
                'ks_date_filter_selection':
                ks_dashboard_rec.ks_date_filter_selection,
                'ks_dashboard_start_date':
                ks_dashboard_rec.ks_dashboard_start_date,
                'ks_dashboard_end_date':
                ks_dashboard_rec.ks_dashboard_end_date,
                'ks_dashboard_top_menu_id':
                ks_dashboard_rec.ks_dashboard_top_menu_id.id,
                'ks_data_formatting':
                ks_dashboard_rec.ks_data_formatting,
            }
    ...
    def ks_import_dashboard(self, file):
        ...
        ks_dashboard_key = [
            'name', 'ks_dashboard_menu_name', 'ks_gridstack_config'
        ]
        ks_dashboard_item_key = [
            'ks_model_id', 'ks_chart_measure_field', 'ks_list_view_fields',
            'ks_record_field', 'ks_chart_relation_groupby', 'ks_id'
        ]
        ...
        for data in ks_dashboard_data:
            ...
            ks_dashboard_top_menu_id = data.get('ks_dashboard_top_menu_id',
                                                False)
            if ks_dashboard_top_menu_id:
                try:
                    self.env['ir.ui.menu'].browse(
                        ks_dashboard_top_menu_id).name
            ...
            vals = {
                ...
                data['name'],
                'ks_dashboard_menu_name':
                data['ks_dashboard_menu_name'],
                'ks_dashboard_top_menu_id':
                ks_dashboard_top_menu_id.id if ks_dashboard_top_menu_id else
                self.env.ref("ks_dashboard_ninja.board_menu_root").id,
                'ks_dashboard_active':
                True,
                'ks_gridstack_config':
                data['ks_gridstack_config'],
                'ks_dashboard_default_template':
                self.env.ref("ks_dashboard_ninja.ks_blank").id,
                'ks_dashboard_group_access':
                False,
                'ks_set_interval':
                data['ks_set_interval'],
                'ks_date_filter_selection':
                data['ks_date_filter_selection'],
                'ks_dashboard_start_date':
                data['ks_dashboard_start_date'],
                'ks_dashboard_end_date':
                data['ks_dashboard_end_date'],
            }
            # Creating Dashboard
            ...
            for id_index, id in enumerate(item_ids):
                if data['ks_gridstack_config'] and str(
                        id) in ks_gridstack_config:
                    if id_index in item_new_ids:
                        ks_grid_stack_config[str(
                            item_new_ids[id_index])] = ks_gridstack_config[str(
                                id)]
    ...
    def ks_create_item(self, item):
        ...
        ks_goal_lines = item['ks_goal_liness'].copy() if item.get(
            'ks_goal_liness', False) else False
        ks_action_lines = item['ks_action_liness'].copy() if item.get(
            'ks_action_liness', False) else False
        ks_dn_header_line = item['ks_dn_header_line'].copy() if item.get(
            'ks_dn_header_line', False) else False
    ...
    def ks_prepare_item(self, item):
        ...
        if item['ks_chart_relation_sub_groupby']:
            ...
            if ks_chart_relation_sub_groupby:
                item[
                    'ks_chart_relation_sub_groupby'] = ks_chart_relation_sub_groupby.id
            else:
        ...
        if item['ks_list_target_deviation_field']:
            ks_list_target_deviation_field = item[
                'ks_list_target_deviation_field']
        ...
        ks_model_id = self.env['ir.model'].search([('model', '=',
                                                    item['ks_model_id'])]).id
        ...
        if (item['ks_model_id_2']):
            ...
            ks_model_id_2 = self.env['ir.model'].search([
        ...
        for model_id in filter_model_ids:
            ...
            for rec in filter_ids:
                ...
                if data.get(rec.ks_model_id.model) and rec.ks_domain:
                    data[rec.ks_model_id.model]['domain'] = data[
                        rec.ks_model_id.model]['domain'] + safe_eval(
                            rec.ks_domain)
                    data[rec.ks_model_id.model]['domain'].insert(
                        0, connect_symbol)
                    ...
                    data[rec.ks_model_id.model] = {
                        'domain':
                        safe_eval(ks_domain) if ks_domain else [],
                        'ks_domain_index_data': [],
                        'model_name':
                        rec.ks_model_id.name,
                        'item_ids':
                        self.env['ks_dashboard_ninja.item'].search(
                            [['id', 'in', self.ks_dashboard_items_ids.ids],
                             '|', ['ks_model_id', '=', rec.ks_model_id.id],
                             ['ks_model_id_2', '=', rec.ks_model_id.id]]).ids
                    }
    ...
    def ks_prepare_dashboard_pre_domain(self):
        ...
        for rec in pre_defined_filter_ids:
            ...
            data[rec['id']] = {
                ...
                rec.name,
                'model':
                rec.ks_model_id.model,
                'model_name':
                rec.ks_model_id.name,
                'active':
                rec.ks_is_active,
                'categ':
                rec.ks_model_id.model + '_' +
                str(categ_seq) if rec.display_type != 'line_section' else 0,
                'type':
                'filter'
                if rec.display_type != 'line_section' else 'separator',
                'domain':
                safe_eval(ks_domain) if ks_domain else [],
                'sequence':
                rec.sequence
    ...
    def ks_prepare_dashboard_custom_domain(self):
        custom_filter_ids = self.env[
            'ks_dashboard_ninja.board_custom_filters'].search(
                [['id', 'in', self.ks_dashboard_custom_filters_ids.ids]],
                order='name')
        data = {}
        for rec in custom_filter_ids:
            ...
            if rec.ks_domain_field_id.ttype == 'selection':
                data[str(rec.id)]['special_data'] = {
                    'select_options':
                    self.env[rec.ks_model_id.model].fields_get(
