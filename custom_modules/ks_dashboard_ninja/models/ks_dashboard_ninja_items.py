    ...
    if self.name == 'ks_list_view_fields' or self.name == 'ks_list_view_group_fields':
        ...
        for record in records:
            ...
            if record.ks_many2many_field_ordering:
                order = json.loads(record.ks_many2many_field_ordering).get(
                    field, False)
...
def ks_time_addition(self, gb, query):
    ...
    return {
        ...
        'groupby': gb,
        'type': field_type,
        'display_format': display_formats[gb_function
                                          or 'month'] if temporal else None,
        'interval': time_intervals[gb_function
                                   or 'month'] if temporal else None,
...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_domain = fields.Char(string="Domain",
                            help="Define conditions for filter. ")
    ...
    ks_default_icon = fields.Char(string="Icon",
                                  default="bar-chart",
    ...
    ks_default_icon_color = fields.Char(
        ...
        string="Icon Color",
        help='Select the icon to be displayed. ')
    ks_icon_select = fields.Char(string="Icon Option",
                                 default="Default",
                                 help='Choose the Icon option. ')
    ks_font_color = fields.Char(default="#ffffff,0.99",
                                string="Font Color",
                                help='Select the font color. ')
    ...
    ks_record_field_2 = fields.Many2one(
        'ir.model.fields',
        domain=
        "[('model_id','=',ks_model_id_2),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
        "'|','|',('ttype','=','integer'),('ttype','=','float'),"
        "('ttype','=','monetary')]",
        string="Kpi Record Field")
    ks_record_count_2 = fields.Float(string="KPI Record Count",
                                     readonly=True,
    ...
    ks_record_count = fields.Float(string="Record Count",
    ...
    ks_record_field = fields.Many2one(
        'ir.model.fields',
        domain=
        "[('model_id','=',ks_model_id),('name','!=','id'),('store','=',True),'|',"
        "'|',('ttype','=','integer'),('ttype','=','float'),"
        "('ttype','=','monetary')]",
        string="Record Field")
    ...
    ks_date_filter_field = fields.Many2one(
        'ir.model.fields',
        domain=
        "[('model_id','=',ks_model_id),('store','=',True),'|',('ttype','=','date'),"
        "('ttype','=','datetime')]",
    ...
    ks_date_filter_field_2 = fields.Many2one(
        'ir.model.fields',
        domain=
        "[('model_id','=',ks_model_id_2),('store','=',True),'|',('ttype','=','date'),"
        "('ttype','=','datetime')]",
    ...
    ks_previous_period = fields.Boolean(
        ...
        help=
        'Checkbox to show comparison between the data of present day and the previous selected period. '
    )
    ...
    ks_chart_relation_groupby = fields.Many2one(
        'ir.model.fields',
        domain=
        "[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
    ...
    ks_chart_date_groupby = fields.Selection(
        [('minute', 'Minute'), ('hour', 'Hour'), ('day', 'Day'),
         ...
         ('year', 'Year'), ('month_year', 'Month-Year')],
    ...
    ks_graph_preview = fields.Char(string="Graph Preview",
                                   default="Graph Preview")
    ks_chart_data = fields.Char(string="Chart Data in string form",
                                compute='ks_get_chart_data',
                                compute_sudo=False)
    ks_chart_data_count_type = fields.Selection([('count', 'Count'),
                                                 ('sum', 'Sum'),
                                                 ('average', 'Average')],
                                                string="Data Type",
                                                default="sum")
    ...
    ks_chart_cumulative_field = fields.Many2one(
        'ir.model.fields',
    ...
    ks_bar_chart_stacked = fields.Boolean(
        string="Stacked Bar Chart",
        help='Stack the columns of the same record. ')
    ...
    ks_sort_by_field = fields.Many2one(
        'ir.model.fields',
        domain=
        "[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
    ...
    ks_list_view_preview = fields.Char(string="List View Preview",
                                       default="List View Preview")
    ...
    ks_kpi_data = fields.Char(string="KPI Data",
                              compute="ks_get_kpi_data",
                              compute_sudo=False)

    ks_chart_item_color = fields.Selection(
        [('default', 'Default'), ('cool', 'Cool'), ('warm', 'Warm'),
         ('neon', 'Neon')],
    ...
    ks_export_all_records = fields.Boolean(
        ...
        default=True,
        help=
        "when click on boolean button, all the records will be downloaded which are present in entire list"
    )
    ...
    ks_list_view_data = fields.Char(string="List View Data in JSon",
    ...
    ks_show_data_value = fields.Boolean(string="Show Data Value",
                                        help=' Display value on the graph. . ')

    ks_action_lines = fields.One2many('ks_dashboard_ninja.item_action',
                                      'ks_dashboard_item_id',
                                      string="Action Lines")

    ks_actions = fields.Many2one('ir.actions.act_window',
    ...
    ks_compare_period = fields.Integer(
        ...
        help=
        ' Provide the number of Date Filter Selection you want to include while displaying the record.'
    )
    ...
    ks_unit = fields.Boolean(string="Show Custom Unit",
                             default=False,
                             help='Display the unit of the data.')
    ...
    ks_show_records = fields.Boolean(string="Show Records",
                                     default=True,
    ...
    ks_domain_extension = fields.Char(
        'Domain Extension',
        help="Define conditions for filter to write manually")
    ks_domain_extension_2 = fields.Char('KPI Domain Extension')
    # hide legend
    ks_hide_legend = fields.Boolean('Show Legend',
                                    help="Hide all legend from the chart item",
                                    default=True)
    ...
    ks_data_format = fields.Selection(
        [('global', 'English Format'), ('indian', 'Indian Format'),
         ('colombian', 'Colombian Peso Format'), ('exact', 'Exact Value')],
    ...
    ks_auto_update_type = fields.Selection(
        ...
        if self._context.get('ks_set_interval', False) else False,
    ...
    @api.depends('ks_multiplier_active', 'ks_chart_measure_field',
    ...
    def ks_date_target_line(self):
        for rec in self:
            if rec.ks_chart_date_groupby in (
                    'minute',
                    'hour') or rec.ks_chart_date_sub_groupby in ('minute',
                                                                 'hour'):
                rec.ks_goal_lines = False
                return {
                    'warning': {
                        'title':
                        _('Groupby Field aggregation'),
                        'message':
                        _('Cannot create target lines when Group By Date field is set to have aggregation in '
    ...
    def copy_data(self, default=None):
        ...
        if 'ks_action_lines' not in default:
            default['ks_action_lines'] = [(0, 0, line.copy_data()[0])
                                          for line in self.ks_action_lines]

        if 'ks_goal_lines' not in default:
            default['ks_goal_lines'] = [(0, 0, line.copy_data()[0])
                                        for line in self.ks_goal_lines]
    ...
    def create(self, values):
        ...
        if values.get('ks_list_view_fields', False) and values.get(
            ...
            ks_many2many_field_ordering = {
                'ks_list_view_fields':
                values['ks_list_view_fields'][0][2],
                'ks_list_view_group_fields':
                values['ks_list_view_group_fields'][0][2],
            }
        ...
        for rec in self:
            ...
            if values.get('ks_list_view_fields', False):
                ks_many2many_field_ordering['ks_list_view_fields'] = values[
                    'ks_list_view_fields'][0][2]
            if values.get('ks_list_view_group_fields', False):
                ks_many2many_field_ordering[
                    'ks_list_view_group_fields'] = values[
                        'ks_list_view_group_fields'][0][2]
            values['ks_many2many_field_ordering'] = json.dumps(
                ks_many2many_field_ordering)
    ...
    def ks_get_dark_color(self, color, opacity):
        ...
        return "#" + hex(0x1000000 + R + G +
                         B).split('x')[1][1:] + "," + opacity
    ...
    @api.onchange('ks_record_count', 'ks_layout', 'name', 'ks_model_id',
                  'ks_domain', 'ks_icon_select', 'ks_default_icon', 'ks_icon',
                  'ks_background_color', 'ks_font_color',
    ...
    @api.depends('ks_record_count_type', 'ks_model_id', 'ks_domain',
                 'ks_record_field', 'ks_date_filter_field', 'ks_item_end_date',
    ...
    def _ksGetRecordCount(self, domain=[]):
        ...
        if rec.ks_record_count_type == 'count' or rec.ks_dashboard_item_type == 'ks_list_view':
            ks_record_count = rec.ks_fetch_model_data(rec.ks_model_name,
                                                      rec.ks_domain,
                                                      'search_count', rec,
                                                      domain)
        elif rec.ks_record_count_type in [
                'sum', 'average'
        ] and rec.ks_record_field and rec.ks_dashboard_item_type != 'ks_list_view':
            ...
            if ks_records_grouped_data and len(ks_records_grouped_data) > 0:
                ...
                if rec.ks_record_count_type == 'sum' and ks_records_grouped_data.get(
                        '__count', False) and (ks_records_grouped_data.get(
                            rec.ks_record_field.name)):
            ...
            if ks_domain and ks_domain != '[]' and ks_model_name:
                ...
                if ks_func == 'search_count':
                    ...
                    data = self.env[ks_model_name].read_group(
                        proper_domain, [rec.ks_record_field.name], [],
                        lazy=False)
            elif ks_model_name:
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec, domain=[]):
        ...
        if ks_domain and "%MYCOMPANY" in ks_domain:
            ks_domain = ks_domain.replace('"%MYCOMPANY"',
        ...
        if rec.ks_date_filter_field:
            if not rec.ks_date_filter_selection or rec.ks_date_filter_selection == "l_none":
                selected_start_date = self._context.get(
                    'ksDateFilterStartDate', False)
                selected_end_date = self._context.get('ksDateFilterEndDate',
                                                      False)
                ...
                ) and self._context['ksDateFilterSelection'] not in [
                        'l_none', 'l_custom'
                ]:
                    ks_date_data = ks_get_date(
                        self._context.get('ksDateFilterSelection'), self,
                    ...
                    if rec.ks_compare_period:
                        ...
                        if rec.ks_compare_period > 0:
                            selected_end_date = selected_end_date + (
                                selected_end_date -
                                selected_start_date) * ks_compare_period
    ...
    @api.onchange('ks_chart_relation_sub_groupby', 'ks_fill_temporal',
                  'ks_goal_lines')
    ...
    def _ks_get_chart_data(self, domain=[]):
        ...
        if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type != 'ks_tile' and \
            ...
            if rec.ks_unit and rec.ks_unit_selection == 'monetary':
                ...
                ks_chart_data[
                    'ks_currency'] += rec.env.user.company_id.currency_id.id
            ...
            if rec.ks_chart_data_count_type == "count":
                ...
                for res in rec.ks_chart_measure_field:
                    ...
                    ks_chart_measure_field_with_type.append(res.name + ':' +
                                                            ks_data_count_type)
            ...
            if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                ...
                if rec.ks_chart_relation_groupby.ttype == 'date' and rec.ks_chart_date_groupby in (
                        'minute', 'hour'):
                    raise ValidationError(
                        _('Groupby field: {} cannot be aggregated by {}').
                        format(rec.ks_chart_relation_groupby.display_name,
                               rec.ks_chart_date_groupby))
                ...
                if (rec.ks_chart_groupby_type == 'date_type' and rec.ks_chart_date_groupby) or \
                    ...
                    ks_chart_data = rec.ks_fetch_chart_data(
                        rec.ks_model_name, ks_chart_domain,
                        ks_chart_measure_field_with_type,
                        ks_chart_measure_field_with_type_2,
                    ...
                    if rec.ks_chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.ks_dashboard_item_type in [
                            'ks_bar_chart', 'ks_horizontalBar_chart',
                        ...
                        if rec.ks_date_filter_selection == "l_none":
                            ...
                            if rec.ks_date_filter_selection == "l_custom":
                                ...
                                ks_date_data = ks_get_date(
                                    rec.ks_date_filter_selection, self,
                                    rec.ks_date_filter_field.ttype)
                                selected_start_date = ks_date_data[
                                    "selected_start_date"]
                                selected_end_date = ks_date_data[
                                    "selected_end_date"]
                        ...
                        if selected_start_date and selected_end_date:
                            ...
                            selected_end_date = selected_end_date.strftime(
                                '%Y-%m-%d')
                        ks_goal_domain = [('ks_dashboard_item', '=',
                                           ks_item_id)]
                        ...
                        ks_date_data = rec.ks_get_start_end_date(
                            rec.ks_model_name, ks_chart_groupby_relation_field,
                            rec.ks_chart_relation_groupby.ttype,
                            ks_chart_domain, ks_goal_domain)
                        ...
                        if rec.ks_chart_date_groupby == 'month_year':
                            ...
                            ks_chart_date_groupby = rec.ks_chart_date_groupby
                        if ks_date_data['start_date'] and ks_date_data[
                                'end_date'] and rec.ks_goal_lines:
                        ...
                        ks_goal_records = self.env[
                            'ks_dashboard_ninja.item_goal'].read_group(
                        ...
                        if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
                            ...
                            for res in ks_goal_records:
                                if res['ks_goal_date' + ":" +
                                    ...
                                    ks_goal_labels.append(
                                        res['ks_goal_date' + ":" +
                                            ks_chart_date_groupby])
                                    ks_goal_dataset.append(
                                        res['ks_goal_value'])
                                    ks_goal_domains[
                                        res['ks_goal_date' + ":" +
                                            ks_chart_date_groupby]] = res[
                                                '__domain']
                            ...
                            for goal_domain in ks_goal_domains.keys():
                                ...
                                for item in ks_goal_domains[goal_domain]:
                                    ...
                                    if 'ks_goal_date' in item:
                                        domain = list(item)
                                        domain[
                                            0] = ks_chart_groupby_relation_field
                                        domain = tuple(domain)
                            ...
                            for label in ks_chart_data['labels']:
                                domains[label] = ks_chart_data['domains'][
                                    counter]
                                counter += 1

                            ks_chart_records_dates = ks_chart_data[
                                'labels'] + list(
                                    set(ks_goal_labels) -
                                    set(ks_chart_data['labels']))
                            ...
                            for label in ks_chart_records:
                                ...
                                if label in ks_chart_data['labels']:
                                    index = ks_chart_data['labels'].index(
                                        label)
                            ...
                            if rec.ks_standard_goal_value:
                                length = len(
                                    ks_chart_data['datasets'][0]['data'])
                                for i in range(length):
                                    goal_dataset.append(
                                        rec.ks_standard_goal_value)
                        ks_goal_datasets = {
                            'label': 'Target',
                            'data': goal_dataset,
                        }
                ...
                (rec.ks_chart_sub_groupby_type == 'relational_type') or
                (rec.ks_chart_sub_groupby_type == 'selection') or
                (rec.ks_chart_sub_groupby_type == 'date_type'
                 and rec.ks_chart_date_sub_groupby) or
                (rec.ks_chart_sub_groupby_type == 'other')):
                if rec.ks_chart_relation_sub_groupby.ttype == 'date':
                    if rec.ks_chart_date_sub_groupby in ('minute', 'hour'):
                        raise ValidationError(
                            _('Sub Groupby field: {} cannot be aggregated by {}'
                              ).format(
                                  rec.ks_chart_relation_sub_groupby.
                                  display_name, rec.ks_chart_date_sub_groupby))
                    ...
                    if rec.ks_chart_date_groupby == 'month_year':
                        ...
                        ks_chart_date_groupby = rec.ks_chart_date_groupby
                if len(ks_chart_measure_field
                       ) != 0 or rec.ks_chart_data_count_type == 'count':
                    ...
                    ks_chart_groupby_relation_fields = [
                        ks_chart_group, ks_chart_sub_groupby_field
                    ]
                    ks_chart_record = False
                    try:
                        ks_chart_record = self.env[
                            rec.ks_model_name].read_group(
                                ks_chart_domain,
                                list(
                                    set(ks_chart_measure_field_with_type +
                                        ks_chart_measure_field_with_type_2 + [
                        ...
                        if res.get(ks_chart_groupby_relation_fields[0], False):
                            if rec.ks_chart_groupby_type == 'date_type':
                                ...
                                if rec.ks_chart_date_groupby == "day" \
                                    ...
                                    label = " ".join(
                                        res[ks_chart_groupby_relation_fields[
                                            0]].split(" ")[0:2])
                                ...
                                label = res[ks_chart_groupby_relation_fields[
                                    0]][1]._value
                            ...
                            if rec.ks_chart_data_count_type != 'count':
                                for ress in rec.ks_chart_measure_field:
                                    if rec.ks_chart_sub_groupby_type == 'date_type':
                                        if res[ks_chart_groupby_relation_fields[
                                                1]] is not False:
                                            labels.append(res[
                                                ks_chart_groupby_relation_fields[
                                                    1]].split(" ")[0] + " " +
                    ...
                    for dataset in final_datasets:
                        ...
                        for label in xlabels:
                            ks_dataset['value'].append({
                                ...
                                if label in values[dataset] else 0
                            })
                    ...
                    if rec.ks_chart_relation_sub_groupby.name == rec.ks_chart_relation_groupby.name == rec.ks_sort_by_field.name:
                        ks_data = rec.ks_sort_sub_group_by_records(
                            ks_data, rec.ks_chart_groupby_type,
                    ...
                    if len(ks_data) != 0:
                            ...
                            for data in chart_sub_data:
                                ...
                                for seri in serie:
                                    ...
                                    if label in values_2[seri]:
                                        values_2[seri][label] = values_2[seri][
                                            label] + value[counter]
                                    else:
                            ...
                            for dataset in final_datasets_2:
                                ...
                                for label in xlabels:
                                    ks_dataset['value'].append({
                                        ...
                                        label,
                                        'y':
                                        values_2[dataset][label]
                                        if label in values_2[dataset] else 0
                                    })
                        ...
                        if rec.ks_goal_enable and rec.ks_standard_goal_value and rec.ks_dashboard_item_type in [
                                'ks_bar_chart', 'ks_line_chart',
                                'ks_area_chart', 'ks_horizontalBar_chart'
                        ]:
                            goal_dataset = []
            ...
            if self.ks_multiplier_active:
                for ks_multiplier in self.ks_multiplier_lines:
                    for i in range(0, len(ks_chart_data['datasets'])):
                        if ks_multiplier.ks_multiplier_fields.field_description in ks_chart_data[
                                'datasets'][i]['label']:
                            data_values = ks_chart_data['datasets'][i]['data']
                            data_values = list(
                                map(
                                    lambda x: ks_multiplier.ks_multiplier_value
                                    * x, data_values))
    ...
    @api.depends(
        'ks_domain', 'ks_dashboard_item_type', 'ks_pagination_limit',
        'ks_model_id', 'ks_sort_by_field', 'ks_sort_by_order',
        'ks_multiplier_active', 'ks_multiplier_lines', 'ks_record_data_limit',
        'ks_list_view_fields', 'ks_list_view_type',
        'ks_list_view_group_fields', 'ks_chart_groupby_type',
        'ks_chart_date_groupby', 'ks_date_filter_field', 'ks_item_end_date',
        'ks_item_start_date', 'ks_compare_period', 'ks_year_period',
        'ks_list_target_deviation_field', 'ks_goal_enable',
        'ks_standard_goal_value', 'ks_goal_lines', 'ks_domain_extension')
    ...
    def get_list_view_record(self,
        ...
        if self.ks_list_view_type == "ungrouped":
            ...
            if self.ks_chart_groupby_type == 'relational_type':
                ...
                ks_list_view_data[
                    'groupby'] = self.ks_chart_relation_groupby.name
                ...
                ks_list_view_data['store'].append(
                    self.ks_chart_relation_groupby.store)
                ...
                if self.ks_goal_enable and self.ks_list_target_deviation_field:
                    ...
                    if self.ks_list_target_deviation_field.name in ks_list_field:
                        ks_list_field.remove(
                            self.ks_list_target_deviation_field.name)
                        ks_list_fields.remove(
                            self.ks_list_target_deviation_field.name)
                        ks_list_view_data['label'].remove(
                            self.ks_list_target_deviation_field.
                            field_description)
                ...
                ks_list_view_records = self.env[self.ks_model_name]. \
                    read_group(ks_chart_domain, ks_list_field + list_target_deviation_field,
                               ...
                               orderby=orderby, limit=limit, offset=ksoffset, lazy=False)
                if all(list_fields in res for res in ks_list_view_records
                       for list_fields in ks_list_fields +
                       list_target_deviation_field):
                    for res in ks_list_view_records:
                        counter = 0
                        data_row = {
                    ...
                    if self.ks_goal_enable:
                        ...
                        for res in ks_list_view_records:
                            ks_list_labels.append(
                                res[ks_list_view_data['groupby']])
                        ks_list_view_data2 = self.get_target_list_view_data(
                            ks_list_view_records, self, ks_list_fields,
                            ks_list_view_data['groupby'],
                            list_target_deviation_field, ks_chart_domain)
                        ks_list_view_data['data_rows'] = ks_list_view_data2[
                            'data_rows']
                        ...
                        for label in ks_list_view_data['label']:
                            if label == 'Achievement':
                                ...
                                if label in ks_label:
                                    ...
                                    ks_fields_type_value = ks_fields_type[
                                        index]
                                    ks_list_view_data['fields_type'].append(
                ...
                for res in ks_list_view_records:
                    if all(list_fields in res
                        ...
                        if res[ks_selection_field]:
                            data_row['data'].append(
                                dict(self.env[self.ks_model_name].fields_get(
            ...
            for ks_multiplier in self.ks_multiplier_lines:
                ...
                if label in ks_list_view_data['label']:
                    ...
                    for i in range(0, len(ks_list_view_data['data_rows'])):
                        data_values = ks_list_view_data['data_rows'][i][
                            'data'][index] * ks_multiplier.ks_multiplier_value
                        ks_list_view_data['data_rows'][i]['data'][
                            index] = data_values
        return ks_list_view_data

    def get_target_list_view_data(self, ks_list_view_records, rec,
                                  ks_list_fields, ks_group_by,
        ...
        for res in ks_list_view_records:
            ...
            for fields in ks_list_fields[1:]:
                ks_list_records[res[ks_group_by]]['measure_field'].append(
                    res[fields])
        ...
        if rec.ks_date_filter_selection_2 == "l_none":
            selected_start_date = rec._context.get('ksDateFilterStartDate',
                                                   False)
        ...
        ks_date_data = rec.ks_get_start_end_date(
            rec.ks_model_name, rec.ks_chart_relation_groupby.name,
            rec.ks_chart_relation_groupby.ttype, ks_chart_domain,
            ks_goal_domain)
        ...
        if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
            ...
            for res in ks_goal_records:
                if res['ks_goal_date' + ":" + ks_chart_date_groupby]:
                    ks_goal_labels.append(res['ks_goal_date' + ":" +
                                              ks_chart_date_groupby])
                    ks_goal_dataset[
                        res['ks_goal_date' + ":" +
                            ks_chart_date_groupby]] = res['ks_goal_value']
                    ks_goal_domains[res['ks_goal_date' + ":" +
                                        ks_chart_date_groupby]] = res.get(
                                            '__domain')
            ...
            for label in ks_list_labels_dates:
                ...
                if data:
                    data_rows[
                        'data'] = data_rows['data'] + data['measure_field']
                ...
                data_rows['ks_column_type'].append(
                    self.ks_chart_relation_groupby.ttype)
    ...
    @api.model
    def ks_fetch_list_view_data(self,
                                rec,
                                ks_chart_domain,
                                limit=15,
                                offset=0,
        ...
        if self.ks_list_view_fields:
            ...
            ks_list_view_fields = [
                res.name for res in self.ks_list_view_fields
            ]
            ks_list_view_field_type = [
                res.ttype for res in self.ks_list_view_fields
            ]
        try:
        ...
        for res in ks_list_view_records:
            ...
            for field_rec in ks_list_view_fields:
                if type(res[field_rec]) == fields.datetime or type(
                        res[field_rec]) == fields.date:
                ...
                data_row['data'].append(res[field_rec])
                data_row['ks_column_type'].append(
                    ks_list_view_field_type[counter])
                counter += 1
            ks_list_view_data['data_rows'].append(data_row)
    ...
    @api.depends('ks_dashboard_item_type', 'ks_goal_enable',
                 'ks_standard_goal_value', 'ks_record_count',
                 'ks_record_count_2', 'ks_previous_period',
                 'ks_compare_period', 'ks_year_period', 'ks_compare_period_2',
                 'ks_year_period_2', 'ks_domain_extension_2')
    ...
    def _ksGetKpiData(self, domain1=[], domain2=[]):
        ...
        if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type == 'ks_kpi' and rec.ks_model_id:
            ...
            ks_kpi_data_model_1[
                'record_field'] = rec.ks_record_field.field_description
            ...
            if rec.ks_previous_period:
                ...
                ks_kpi_data_model_1[
                    'previous_period'] = ks_previous_period_data
            ...
            if rec.ks_model_id_2 and rec.ks_record_count_type_2:
                ...
                ks_kpi_data_model_2['record_data'] = rec._ksGetRecordCount_2(
                    domain2)
    ...
    def ks_get_previous_period_data(self, rec):
        ...
        if (ks_date_data):
            ...
            proper_domain = rec.ks_get_previous_period_domain(
                rec.ks_domain, previous_period_start_date,
            ...
            if rec.ks_record_count_type == 'count':
                ...
                if rec.ks_record_count_type == 'sum':
                    return data.get(rec.ks_record_field.name, 0) if data.get(
                        '__count', False) and (data.get(
    ...
    def ks_get_previous_period_domain(self, ks_domain, ks_start_date,
                                      ks_end_date, date_filter_field):
        if ks_domain and "%UID" in ks_domain:
            ks_domain = ks_domain.replace('"%UID"', str(self.env.user.id))
        if ks_domain:
            # try:
            ...
            if ks_start_date and ks_end_date and date_filter_field:
                proper_domain = ([(date_filter_field.name, ">=",
                                   ks_start_date),
                                  (date_filter_field.name, "<=", ks_end_date)])
            else:
                proper_domain = []
    ...
    @api.depends('ks_domain_2', 'ks_model_id_2', 'ks_record_field_2',
                 'ks_record_count_type_2', 'ks_item_start_date_2',
    ...
    def _ksGetRecordCount_2(self, domain=[]):
        ...
        if rec.ks_record_count_type_2 == 'count':
            ks_record_count = rec.ks_fetch_model_data_2(
                rec.ks_model_name_2, rec.ks_domain_2, 'search_count', rec,
                domain)

        elif rec.ks_record_count_type_2 in ['sum', 'average'
                                            ] and rec.ks_record_field_2:
            ks_records_grouped_data = rec.ks_fetch_model_data_2(
                rec.ks_model_name_2, rec.ks_domain_2, 'read_group', rec,
                domain)
            if ks_records_grouped_data and len(ks_records_grouped_data) > 0:
                ks_records_grouped_data = ks_records_grouped_data[0]
                if rec.ks_record_count_type_2 == 'sum' and ks_records_grouped_data.get(
                        '__count', False) and (ks_records_grouped_data.get(
                            rec.ks_record_field_2.name)):
                    ks_record_count = ks_records_grouped_data.get(
    ...
    # Writing separate function to fetch dashboard item data
    def ks_fetch_model_data_2(self,
                              ks_model_name,
                              ks_domain,
                              ks_func,
                              rec,
        ...
        try:
            if ks_domain and ks_domain != '[]' and ks_model_name:
                ...
                if ks_func == 'search_count':
                    ...
                    data = self.env[ks_model_name].read_group(
                        proper_domain, [rec.ks_record_field_2.name], [],
                        lazy=False)
            elif ks_model_name:
                # Have to put extra if condition here because on load,model giving False value
                proper_domain = self.ks_convert_into_proper_domain_2(
    ...
    def ks_set_date_filter_2(self):
        for rec in self:
            if (not rec.ks_date_filter_selection_2
    ...
    def ks_convert_into_proper_domain_2(self, ks_domain_2, rec, domain=[]):
        ...
        if ks_domain_2 and "%MYCOMPANY" in ks_domain_2:
            ks_domain_2 = ks_domain_2.replace('"%MYCOMPANY"',
                                              str(self.env.company.id))

        ks_date_domain = False

        if not rec.ks_date_filter_selection_2 or rec.ks_date_filter_selection_2 == "l_none":
            selected_start_date = self._context.get('ksDateFilterStartDate',
                                                    False)
            ...
            if self._context.get(
                    ...
                    False) and self._context['ksDateFilterSelection'] not in [
                        'l_none', 'l_custom'
                    ]:
                ks_date_data = ks_get_date(
                    self._context.get('ksDateFilterSelection'), self,
            ...
            if selected_start_date and selected_end_date:
                if rec.ks_compare_period_2:
                    ...
                    if rec.ks_compare_period_2 > 0:
                        selected_end_date = selected_end_date + (
                            selected_end_date -
                            selected_start_date) * ks_compare_period_2
    ...
    def ks_fetch_chart_data(self, ks_model_name, ks_chart_domain,
                            ks_chart_measure_field_with_type,
                            ks_chart_measure_field_with_type_2,
                            ks_chart_measure_field, ks_chart_measure_field_2,
                            ks_chart_groupby_relation_field,
                            ks_chart_date_groupby, ks_chart_groupby_type,
                            orderby, limit, chart_count,
                            ks_chart_measure_field_ids,
                            ks_chart_measure_field_2_ids,
        ...
        for res in ks_chart_records:
            is_ks_index = False
            ks_index = False
            if all(measure_field in res
                   for measure_field in ks_chart_measure_field):
                if ks_chart_groupby_type == "relational_type":
                    ...
                    if ks_chart_measure_field_2:
                        ...
                        for field_rec in ks_chart_measure_field_2:
                            ...
                            try:
                                ...
                                data = 0
                            ks_chart_data['datasets'][counter]['data'].append(
                                data)
                            counter += 1
                            index += 1
    ...
    def ks_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        if record.ks_unit and record.ks_unit_selection == 'monetary':
            ...
            ks_chart_data[
                'ks_currency'] += record.env.user.company_id.currency_id.id
        ...
        if action_line.ks_chart_type == 'ks_list_view':
            ...
            if action_line.ks_item_action_field.ttype == 'many2one':
                ks_list_view_data['list_view_type'] = 'relational_type'
                ks_list_view_data[
                    'groupby'] = action_line.ks_item_action_field.name
                ...
                for res in ks_list_view_records:
                    ...
                    data_row = {
                        'id':
                        res[action_line.ks_item_action_field.name][0]
                        if res[action_line.ks_item_action_field.name] else
                        res[action_line.ks_item_action_field.name],
                        'data': [],
                        'domain':
                        json.dumps(res['__domain']),
                        'sequence':
                        sequence + 1,
                        'last_seq':
                        ks_count,
                    ...
                    for field_rec in ks_list_fields:
                        if counter == 0:
                            data_row['data'].append(
                                res[field_rec][1].
                                _value if res[field_rec] else "False")
                        else:
                            data_row['data'].append(res[field_rec])
                        counter += 1
                        data_row['ks_column_type'].append(
                ...
                ks_list_field.append(action_line.ks_item_action_field.name +
                                     ':' +
                ...
                for res in ks_list_view_records:
                    ...
                    if res[ks_selection_field]:
                        data_row['data'].append(
                            dict(self.env[record.ks_model_name].fields_get(
                                allfields=ks_selection_field)
                                 [ks_selection_field]['selection'])[
                                     res[ks_selection_field]])
                    else:
            ...
            ks_chart_data = record.ks_fetch_chart_data(
                record.ks_model_name, domain, ks_chart_measure_field_with_type,
                ks_chart_measure_field_with_type_2, ks_chart_measure_field,
                ks_chart_measure_field_2, ks_chart_groupby_relation_field,
                ks_chart_date_group_by, ks_chart_relation_type, orderby, limit,
                record.ks_chart_data_count_type, ks_chart_measure_field_ids,
                ks_chart_measure_field_2_ids,
                ks_chart_groupby_relation_field_id, ks_chart_data)
            if record.ks_multiplier_active:
                for ks_multiplier in record.ks_multiplier_lines:
    ...
    @api.model
    def ks_get_start_end_date(self, model_name,
                              ks_chart_groupby_relation_field, ttype,
                              ks_chart_domain, ks_goal_domain):
        ks_start_end_date = {}
        try:
        ...
        if model_field_start_date and ttype == "date":
            model_field_end_date = datetime.combine(model_field_end_date,
                                                    datetime.min.time())
            model_field_start_date = datetime.combine(model_field_start_date,
                                                      datetime.min.time())

        if model_field_start_date and goal_model_start_date:
            goal_model_start_date = datetime.combine(goal_model_start_date,
                                                     datetime.min.time())
            goal_model_end_date = datetime.combine(goal_model_end_date,
                                                   datetime.max.time())
            if model_field_start_date < goal_model_start_date:
                ks_start_end_date[
                    'start_date'] = model_field_start_date.strftime(
                        "%Y-%m-%d 00:00:00")
            else:
                ks_start_end_date[
                    'start_date'] = goal_model_start_date.strftime(
                        "%Y-%m-%d 00:00:00")
            ...
            ks_start_end_date['start_date'] = goal_model_start_date.strftime(
                "%Y-%m-%d 00:00:00")
            ks_start_end_date['end_date'] = goal_model_start_date.strftime(
                "%Y-%m-%d 23:59:59")
        else:
    ...
    def ks_sort_sub_group_by_records(self, ks_data, field_type,
                                     ks_chart_date_groupby, ks_sort_by_order,
                                     ks_chart_date_sub_groupby):
        if ks_data:
            reverse = False
                    ...
                    if ks_chart_date_groupby in ['minute', 'hour']:
                        if ks_chart_date_sub_groupby in [
                            ...
                            data['value'].sort(key=lambda x: int(
                                str(
                                    ks_sorted_months.index(x['x'].split(" ")[
                                        2]) + 1) + x['x'].split(" ")[1] +
                                x['x'].split(" ")[0].replace(":", "")),
                                               reverse=reverse)
                        else:
                            data['value'].sort(
                                ...
                                reverse=reverse)
                    elif ks_chart_date_groupby == 'day' and ks_chart_date_sub_groupby in [
                            "quarter", "year"
                    ]:
                        ks_sorted_days = self.generate_timeserise(
                            ...
                            'day', "date")
                        b = [
                            " ".join(x.split(" ")[0:2]) for x in ks_sorted_days
                        ]
                        data['value'].sort(key=lambda x: b.index(x['x']),
                                           reverse=reverse)
                    elif ks_chart_date_groupby == 'day' and ks_chart_date_sub_groupby not in [
                            "quarter", "year"
                    ]:
                        data['value'].sort(key=lambda i: int(i['x']),
                                           reverse=reverse)
                    elif ks_chart_date_groupby == 'week':
                        data['value'].sort(key=lambda i: int(i['x'][1:]),
                                           reverse=reverse)
        ...
        if self.ks_domain_2:
            ...
            try:
                ...
                if "%UID" in ks_domain_2:
                    ks_domain_2 = ks_domain_2.replace("%UID",
                ...
                for element in ks_domain_2:
                    proper_domain_2.append(element) if type(
                        element) != list else proper_domain_2.append(
                            tuple(element))
    ...
    def ks_onchange_check_domain_onchange(self):
        if self.ks_domain:
            ...
            try:
                ...
                if "%UID" in ks_domain:
                    ks_domain = ks_domain.replace("%UID",
                                                  str(self.env.user.id))
                ...
                for element in ks_domain:
                    proper_domain.append(element) if type(
                        element) != list else proper_domain.append(
                            tuple(element))
...
class KsDashboardItemsActions(models.Model):
    ...
    ks_dashboard_item_id = fields.Many2one('ks_dashboard_ninja.item',
                                           string="Dashboard Item")
    ks_model_id = fields.Many2one('ir.model',
                                  related='ks_dashboard_item_id.ks_model_id')
    ...
    ks_sort_by_field = fields.Many2one(
        ...
        "('ttype','!=','binary')]",
        string="Sort By Field")
    ks_sort_by_order = fields.Selection([('ASC', 'Ascending'),
                                         ('DESC', 'Descending')],
                                        string="Sort Order")
    ...
    def ks_check_date_group_by(self):
        for rec in self:
            if rec.ks_item_action_field.ttype == 'date' and rec.ks_item_action_date_groupby in [
                    'hour', 'minute'
            ]:
                raise ValidationError(
    ...
    def ks_onchange_item_action(self):
        for rec in self:
            if not (rec.ks_item_action_field.ttype == 'datetime'
